...
def getSizeTensor(h, w):
    if type(h) == int:
        res = torch.Tensor([[w / 2.0, 0., w / 2.0],
                            [0., h / 2.0, h / 2.0],
                            [0., 0., 1.]]).unsqueeze(0)
    elif isinstance(h, torch.Tensor):
        res = torch.zeros((h.shape[0], 3, 3))
        res[:, 0, 0] = res[:, 0, 2] = w / 2.0
        res[:, 1, 1] = res[:, 1, 2] = h / 2.0
        res[:, 2, 2] = 1.
    else:
        raise TypeError
...
def transformer(U, theta, out_size, returnGrid = False, **kwargs):
    """
    theta: H
    """
    ...
    def _interpolate(im, x, y, out_size):
        ...
        dim2 = dim2.to(x.device)
        dim1 = dim1.to(x.device)
        # y0 = y0.cuda()
        # y1 = y1.cuda()
        # x0 = x0.cuda()
        # x1 = x1.cuda()
    ...
    def _transform(theta, input_dim, out_size):
        ...
        out_height, out_width = out_size[0], out_size[1]
        grid = _meshgrid(out_height, out_width).to(theta.device)
        ...
        if returnGrid:
            res = torch.stack([x_s_flat, y_s_flat], dim = -1).reshape((num_batch, out_height, out_width, 2))
            return output, res
