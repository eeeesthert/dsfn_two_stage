...
def homography_warp(
	source: torch.Tensor, H_source_to_target: torch.Tensor, output_size=None, origin=None
):
	...
	z = q[:, 2:3]
	z = torch.where(
		z.abs() < 1e-6, torch.where(z < 0, -torch.full_like(z, 1e-6), torch.full_like(z, 1e-6)), z
	)
