...
class Regressor(nn.Module):
	def __init__(self, channels, strides, spatial, hidden, dropout):
		...
		for ci, co, st in zip(channels[:-1], channels[1:], strides):
			layers += [nn.Conv2d(ci, co, 3, stride=st, padding=1), nn.ReLU(inplace=True)]
...
class HomographyNet(nn.Module):
	...
	def forward(self, image1, image2):
		a = F.interpolate(image1, size=(128, 128), mode="bilinear", align_corners=False).mean(
			1, keepdim=True
		)
		b = F.interpolate(image2, size=(128, 128), mode="bilinear", align_corners=False).mean(
			1, keepdim=True
		)
